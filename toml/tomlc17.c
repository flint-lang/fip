...
static toml_datum_t *arr_emplace(toml_datum_t *arr, const char **reason) {
    ...
    toml_datum_t *elem =
        REALLOC(arr->u.arr.elem, sizeof(*elem) * align8(n + 1));
    if (!elem) {
        *reason = "out of memory";
        return NULL;
    }
...
static bool datum_equiv(toml_datum_t a, toml_datum_t b) {
    ...
    switch (a.type) {
        ...
        case TOML_TIME:
            return a.u.ts.hour == b.u.ts.hour &&
                a.u.ts.minute == b.u.ts.minute &&
    ...
    if (toml_option.check_utf8) {
        ...
        for (int i = 0; i < len;) {
            ...
            if (0xD800 <= ch && ch <= 0xDFFF) {
                // explicitly prohibit surrogates (non-scalar unicode code
                // point)
                snprintf(result.errmsg, sizeof(result.errmsg),
...
static int parse_key(parser_t *pp, token_t tok, keypart_t *ret_keypart) {
    ...
    while (1) {
        ...
        if (tok.toktyp != TOK_STRING && tok.toktyp != TOK_LITSTRING &&
            tok.toktyp != TOK_LIT) {
            return RETERROR(pp->ebuf, tok.lineno,
                "expects a string in dotted-key");
        }
...
static toml_datum_t *descend_keypart(parser_t *pp, int lineno,
    toml_datum_t *toptab, keypart_t *keypart, bool stdtabexpr) {
...
static int parse_inline_array(parser_t *pp, token_t tok,
    ...
    *ret_datum = mkdatum(TOML_ARRAY);
    ...
    for (;;) {
        ...
        if (!pelem) {
            return RETERROR(pp->ebuf, tok.lineno, "while parsing array: %s",
                reason);
        }
...
// Parse a value.
static int parse_val(parser_t *pp, token_t tok, toml_datum_t *ret) {
    // val = string / boolean / array / inline-table / date-time / float /
    // integer
    switch (tok.toktyp) {
        case TOK_STRING:
        case TOK_MLSTRING:
...
static int parse_array_table_expr(parser_t *pp, token_t tok) {
    ...
    for (int i = 0; i < keypart.nspan; i++) {
        ...
        // If value is an array of table, point to the last element of the array
        // and continue descent.
        if (value->type == TOML_ARRAY) {
            ...
            if (value->type != TOML_TABLE) {
                return RETERROR(pp->ebuf, keylineno,
                    "array %s must be array of tables", curkey.ptr);
            }
            tab = value;
            continue;
        }
...
static bool is_valid_date(int year, int month, int day) {
    ...
    int days_in_month[] = {31, 28 + is_leap_year, 31, 30, 31, 30, 31, 31, 30,
        31, 30, 31};
...
again:
    *tok = mktoken(sp, TOK_FIN);
    ...
    switch (ch) {
        ...
        default:
            sp->cur--;
            DO(keymode ? scan_literal(sp, tok)
                       : scan_nonstring_literal(sp, tok));
            break;
    }
